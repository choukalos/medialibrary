...
//	  if (is_null($mediafiletypes)) { $this->mediafiletypes = $mediafiletypes; }
	  ...
	  $location = '/Users/cchoukalos/Sites/project2/public/assets/img/thumbs';
	  $this->thumbnailer    = new \Thumbnail($ffmpeglocation,$location,"00:00:04.1", "160x120");
      return $this;
	}
	  ...
	  while (false !== ($fname = readdir($h_dir))) {
		...
		if (is_dir($basedir . $fname)) {
		  ...
		  if (in_array($fileext,$this->mediafiletypes)) {
			...
			$thumbfilename = $obj->create($basedir . $fname);
		    echo " > $basename $fileext $duration $width $height $dirname $thumbfilename\n";			
		  }
	    }
		// end of while loop
	  }  
